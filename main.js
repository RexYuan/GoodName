$(document).ready(function(){
    ...
    $('.generate').on('click', function(){
        tmp = golden_sound[Math.floor(Math.random()*golden_sound.length)];
        for (var i=0, len=Math.floor(Math.random()*3)+1; i<len; i++)
        {
            tmp = tmp + ' ' + golden_sound[Math.floor(Math.random()*golden_sound.length)];
        }
        $('.candidate').text(tmp);
    });
